//
//  InformationSectionController.swift
//  Bloomr
//
...
class InformationSectionController: ListSectionController, ListSupplementaryViewSource {
    lazy var titleHeader: UILabel = {
        let label = UILabel(frame: .zero)
        label.font = UIFont.fromType(.primary(.bold, .h1))
        label.textColor = UIColor.charcoal_grey
        label.text = TextManager.informationAccountText.localized()
        return label
    }()
    
    var items = [
        SettingItem(title: TextManager.profileText.localized(), type: .account),
        SettingItem(title: TextManager.changePasswordText.localized(), type: .account)
    ]
