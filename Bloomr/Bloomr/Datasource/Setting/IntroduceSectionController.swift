//
//  IntroduceSectionController.swift
//  Bloomr
//
...
import IGListKit
class IntroduceSectionController: ListSectionController, ListSupplementaryViewSource {
    lazy var titleHeader: UILabel = {
        let label = UILabel(frame: .zero)
        label.font = UIFont.fromType(.primary(.bold, .h1))
        label.textColor = UIColor.charcoal_grey
        label.text = TextManager.introduceText.localized()
        return label
    }()
    
    var items = [
        SettingItem(title: TextManager.faqText.localized(), type: .account),
        SettingItem(title: TextManager.communityText.localized(), type: .account),
        SettingItem(title: TextManager.tempConditionText.localized(), type: .account)
    ]
