//
//  PrivacySectionController.swift
//  Bloomr
//
...
class PrivacySectionController: ListSectionController, ListSupplementaryViewSource {
    lazy var titleHeader: UILabel = {
        let label = UILabel(frame: .zero)
        label.font = UIFont.fromType(.primary(.bold, .h1))
        label.textColor = UIColor.charcoal_grey
        label.text = TextManager.privacyText.localized()
        return label
    }()
    
    var items = [
        SettingItem(title: TextManager.settingText.localized(), type: .account),
        SettingItem(title: TextManager.blockListText.localized(), type: .account)
    ]
