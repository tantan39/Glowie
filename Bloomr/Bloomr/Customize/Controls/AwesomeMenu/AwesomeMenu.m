...
- (void)AwesomeMenuItemTouchesEnd:(AwesomeMenuItem *)item
{
    ...
    if (item == _startButton) 
    {
        ...
        {
            [_delegate awesomeMenu:self didSelectIndex:-1];
        }
    }
...
- (void)_setMenu {
    ...
    for (int i = 0; i < count; i ++)
    {
        ...
        if (_isCell && i == 0) {
            item.endPoint = CGPointMake(startPoint.x - 45, startPoint.y - 41);//185-160
        } else if (_isCell && i == 1) {
            item.endPoint = CGPointMake(startPoint.x - 5, startPoint.y - 41);
        } else if (_isCell && i == 2) {
            item.endPoint = CGPointMake(startPoint.x + 35, startPoint.y - 41);
        } else if (_isCell && i == 3) {
            item.endPoint = CGPointMake(startPoint.x + 75, startPoint.y - 41);
        } else if (_isCell && i == 4) {
            item.endPoint = CGPointMake(startPoint.x + 115, startPoint.y - 41);
        } else {
...
//        NSLog(@"WI: %f, HE: %f", item.frame.size.width, item.frame.size.height);
//        NSLog(@"XEnd: %f, YEnd: %f", item.endPoint.x, item.endPoint.y);
//        NSLog(@"XNear: %f, YNear: %f", item.nearPoint.x, item.nearPoint.y);
//        NSLog(@"XFair: %f, YFair: %f", item.farPoint.x, item.farPoint.y);
