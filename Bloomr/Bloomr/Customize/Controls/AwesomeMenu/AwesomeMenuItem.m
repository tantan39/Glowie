       ...
       text:(NSString *)text
{
    if (self = [super init]) 
    {
        ...
        _content = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 31, 13)];
        [_content setText:text];
        [_content setFont:[UIFont fontWithName:@"SourceSansPro-Bold" size:17]];
        [_content setTextColor:[UIColor whiteColor]];
        [_content setBackgroundColor:[UIColor clearColor]];
        [_content setTextAlignment:NSTextAlignmentCenter];
...
- (void)layoutSubviews
{
    ...
    float width = _content.frame.size.width;
    float height = _content.frame.size.height;
    _content.frame = CGRectMake(self.bounds.size.width / 2 - width / 2, self.bounds.size.height/2 - height / 2, width, height);
}
...
- (void)setHighlighted:(BOOL)highlighted
{
    ...
    [_content setHighlighted:highlighted];
}
