...
- (void) removeAllAllowedDropLocations{
    [self.droppableLocations removeAllObjects];
}

- (void) removeAllowedDropLocation:(SEDraggableLocation *)location
{
    [self.droppableLocations removeObject:location];
}
...
{
    ((UIImageView*)self.subviews[0]).highlighted = YES;
}
...
- (void) handleDrag:(id)sender {
  ...
  CGPoint myCoordinates   = [self.panGestureRecognizer locationInView:self.superview];
    ...
    ((UIImageView*)self.subviews[0]).highlighted = NO;
