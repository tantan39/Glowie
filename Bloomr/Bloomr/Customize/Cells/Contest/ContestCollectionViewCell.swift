...
protocol ContestCollectionViewCellDelegate: NSObjectProtocol {
    func infoButtonPressed()
    func uploadButtonPressed()
}

extension ContestCollectionViewCellDelegate {
    func infoButtonPressed() {}
    func uploadButtonPressed() {}
}
...
//        imgView.image = UIImage(named: "banner_contest")
    ...
    private let contestLabel: UILabel = {
        let label = UILabel(frame: .zero)
        label.font = .fromType(.primary(.regular, .h4_10))
        label.text = "Contest".localized()
        return label
    }()
    ...
    weak var delegate: ContestCollectionViewCellDelegate?
...
extension ContestCollectionViewCell {
